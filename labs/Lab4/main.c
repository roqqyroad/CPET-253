...
void main(void)
{
    ...
    Port2_Init();
    Port3_Init();
    Port5_Init();// call port initialization functions
    BumpInt_Init(); //Initialize interrupts
    EnableInterrupts();

    enum motor_states {FORWARD, BACKWARD, TURNLEFT, TURNRIGHT} state, prevState;//declare enumerated states, declare starting state, declare previous state, declare state timer
    state = FORWARD;
    prevState = !FORWARD;
    ...
    while(1)
    {
        ...
        switch (state) {
            case FORWARD:
                Motor_Forward(13973, 14999); //call driving function
                if (wasInterrupt){ //if interrupt go backward
                    wasInterrupt = false;
                    state = BACKWARD;
                }
            break;
            case BACKWARD:
                if(isNewState){
                    ...
                    Motor_Backward(13973, 14999); //call driving function
                }
            ...
            case TURNRIGHT:
                if (isNewState){
                    stateTimer = 0; //reset timer
                    Motor_Right(14999, 0); //call driving function
                }
                stateTimer++;
                if (stateTimer >= 91){
                    P4IFG &= 0x00;
            ...
            case TURNLEFT:
                if (isNewState){
                    stateTimer = 0; //reset timer
                    Motor_Left(0, 14999); //call driving function
                }
                stateTimer++;
                if (stateTimer >= 91){
                    P4IFG &= 0x00;
            ...
            break;
            state = FORWARD; //create default statement for switch
        } //end switch
